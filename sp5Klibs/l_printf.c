...
int i;
	...
	i = frtos_write(fdRS232, (char *)stdout_buff, PRINTF_BUFFER_SIZE );
...
//		return(bytes2wr);
	...
	frtos_ioctl (fdRS232,ioctl_OBTAIN_BUS_SEMPH, NULL );
	bytes2wr = frtos_write( fdRS232, pvBuffer, xBytes );
	frtos_ioctl (fdRS232,ioctl_RELEASE_BUS_SEMPH, NULL);
	return(bytes2wr);
