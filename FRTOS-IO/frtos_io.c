...
int8_t xRet = -1;
	...
	switch(fd) {
	case fdRS232:
		xRet = frtos_uart_open( &xComRS232, fd, &RS232_xMutexBuffer, iUART_RS232, flags );
		break;
	case fdGPRS:
...
int8_t xRet = -1;
	...
	switch(fd) {
	case fdRS232:
		xRet = frtos_uart_ioctl( &xComRS232, ulRequest, pvValue );
		break;
	case fdGPRS:
...
int8_t xRet = -1;
	...
	switch(fd) {
	case fdRS232:
		xRet = frtos_uart_write( &xComRS232, pvBuffer, xBytes );
		break;
	case fdGPRS:
...
int8_t xRet = -1;
	...
	switch(fd) {
	case fdRS232:
		xRet = frtos_uart_read( &xComRS232, pvBuffer, xBytes );
		break;
	case fdGPRS:
